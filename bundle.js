...
const build = ({ dest, entry, format, plugins }) => new Promise((resolve, reject) => {
  rollup({ entry, plugins })
    .then(bundle => bundle.write({ dest, format, moduleName: 'WildernessDomNode' })
    .then(resolve))
    .catch(reject)
})
...
Promise.resolve(console.log('Creating UMD development bundle ...'))
  .then(() => build({
    dest: 'dist/wilderness-dom-node.development.js',
    entry: 'src/index.js',
    format: 'umd',
    plugins: [
      babel({
  ...
  .then(() => build({
    dest: 'dist/wilderness-dom-node.production.js',
    entry: 'src/index.js',
    format: 'umd',
    plugins: [
      babel({
